...
public class SachleitendeVerfuegung {
    ...
    public static final String GROUP_ID_SLV_ALL_VERSIONS = "SLV_" + BLOCKNAME_SLV_ALL_VERSIONS;

    public static final String GROUP_ID_SLV_ORIGINAL_ONLY = "SLV_" + BLOCKNAME_SLV_ORIGINAL_ONLY;

    public static final String GROUP_ID_SLV_NOT_IN_ORIGINAL = "SLV_" + BLOCKNAME_SLV_NOT_IN_ORIGINAL;

    public static final String GROUP_ID_SLV_DRAFT_ONLY = "SLV_" + BLOCKNAME_SLV_DRAFT_ONLY;

    public static final String GROUP_ID_SLV_COPY_ONLY = "SLV_" + BLOCKNAME_SLV_COPY_ONLY;
     ...
     */
    public static XTextRange insertVerfuegungspunkt(final TextDocumentController documentController,
            final XTextRange range) {
        if (range == null) return null;

        // Notwendige Absatzformate definieren (falls nicht bereits definiert)
        createUsedStyles(documentController.getModel().doc);

        final XParagraphCursor cursor = UNO.XParagraphCursor(range.getText().createTextCursorByRange(range));

        // Enthält der markierte Bereich bereits Verfuegungspunkte, so werden diese
        // gelöscht
        final boolean deletedAtLeastOne = removeAllVerfuegungspunkte(cursor);
     ...
     *         null, falls der ViewCursor unverändert bleibt.
     */
    public static XTextRange insertAbdruck(final TextDocumentController documentController, final XTextRange range) {
        if (range == null) return null;
        ...
        // Enthält der markierte Bereich bereits Verfuegungspunkte, so werden diese
        // gelöscht
        final boolean deletedAtLeastOne = removeAllAbdruecke(cursor);
        ...
        if (!deletedAtLeastOne) {
            ...
            final int count = countVerfPunkteBefore(documentController.getModel().doc, cursor) + 1;
     ...
     *         null, falls der ViewCursor unverändert bleibt.
     */
    public static XTextRange insertZuleitungszeile(final TextDocumentController documentController,
            final XTextRange range) {
        if (range == null) return null;
        ...
        final boolean deletedAtLeastOne = removeAllZuleitungszeilen(cursor);
        ...
        if (!deletedAtLeastOne && UNO.XEnumerationAccess(cursor) != null) {
            ...
            // setzen.
            final XEnumeration paragraphs = UNO.XEnumerationAccess(cursor).createEnumeration();
     ...
     */
    private static boolean removeAllVerfuegungspunkte(final XParagraphCursor cursor) {
        boolean deletedAtLeastOne = false;
        if (UNO.XEnumerationAccess(cursor) != null) {
            final XEnumeration xenum = UNO.XEnumerationAccess(cursor).createEnumeration();
            ...
            while (xenum.hasMoreElements()) {
                ...
                if (par != null) {
                    final boolean isVerfuegungspunktMitZuleitung = isVerfuegungspunktMitZuleitung(par);
     ...
     */
    private static boolean removeAllAbdruecke(final XParagraphCursor cursor) {
        boolean deletedAtLeastOne = false;
        if (UNO.XEnumerationAccess(cursor) != null) {
            final XEnumeration xenum = UNO.XEnumerationAccess(cursor).createEnumeration();
     ...
     */
    private static boolean removeAllZuleitungszeilen(final XParagraphCursor cursor) {
        boolean deletedAtLeastOne = false;
        if (UNO.XEnumerationAccess(cursor) != null) {
            final XEnumeration xenum = UNO.XEnumerationAccess(cursor).createEnumeration();
     ...
     */
    private static void removeSingleVerfuegungspunkt(final XTextRange par) {
        formatDefault(par);

        // Prüfe, ob der Absatz mit einer römischen Ziffer beginnt.
        final XTextCursor zifferOnly = getZifferOnly(par, false);
        ...
        if (isAbdruck(par)) {
            ...
            final XParagraphCursor parDeleter = UNO.XParagraphCursor(par.getText().createTextCursorByRange(par.getEnd()));
     ...
     * @param paragraph
     */
    private static void formatDefault(final XTextRange paragraph) {
     ...
     * @param paragraph
     */
    private static void formatAbdruck(final XTextRange paragraph) {
     ...
     * @param paragraph
     */
    private static void formatVerfuegungspunktWithZuleitung(final XTextRange paragraph) {
     ...
     * @param paragraph
     */
    private static void formatVerfuegungspunkt(final XTextRange paragraph) {
     ...
     * @param paragraph
     */
    private static void formatZuleitungszeile(final XTextRange paragraph) {
     ...
     * @param paragraph
     */
    private static void formatRoemischeZifferOnly(final XTextRange paragraph) {
        final XTextCursor zifferOnly = getZifferOnly(paragraph, false);
        if (zifferOnly != null) {
     ...
     *         beginnt.
     */
    private static boolean isVerfuegungspunkt(final XTextRange paragraph) {
        String paraStyleName = "";
        try {
            paraStyleName = AnyConverter.toString(UNO.getProperty(paragraph, "ParaStyleName"));
     ...
     */
    private static boolean isVerfuegungspunktMitZuleitung(final XTextRange paragraph) {
        String paraStyleName = "";
        try {
            paraStyleName = AnyConverter.toString(UNO.getProperty(paragraph, "ParaStyleName"));
        } catch (final IllegalArgumentException e) {
     ...
     *         beginnt.
     */
    private static boolean isZuleitungszeile(final XTextRange paragraph) {
        String paraStyleName = "";
        try {
            paraStyleName = AnyConverter.toString(UNO.getProperty(paragraph, "ParaStyleName"));
        } catch (final IllegalArgumentException e) {
     ...
     * @return
     */
    private static boolean isAbdruck(final XTextRange paragraph) {
        final String str = paragraph.getString();
     ...
     *         hinzugefügt wurde.
     */
    private static String getAbdruckSuffix(final XTextRange paragraph) {
        final String str = paragraph.getString();
     ...
     */
    public static int countVerfPunkteBefore(final XTextDocument doc, final XParagraphCursor range) {
        int count = 0;

        // Zähler für Verfuegungspunktnummer auf 1 initialisieren, wenn ein
        // Verfuegungspunkt1 vorhanden ist.
        final XTextRange punkt1 = getVerfuegungspunkt1(doc);
        if (punkt1 != null) count++;

        final XParagraphCursor cursor = UNO.XParagraphCursor(range.getText().createTextCursorByRange(range.getStart()));
        if (cursor != null) do {
     ...
     */
    public static void ziffernAnpassen(final TextDocumentController documentController) {
        final XTextRange punkt1 = getVerfuegungspunkt1(documentController.getModel().doc);
        ...
        final XParagraphCursor cursor = UNO.XParagraphCursor(documentController.getModel().doc.getText().createTextCursorByRange(
        ...
        if (cursor != null) do {
            ...
            if (isVerfuegungspunkt(cursor)) {
                ...
                if (isAbdruck(cursor)) {
                    ...
                    final String abdruckStr = abdruckString(count) + getAbdruckSuffix(cursor);
                    ...
                    final String numberStr = romanNumber(count) + "\t";
        ...
        if (punkt1 != null) {
            final XTextRange zifferOnly = getZifferOnly(punkt1, false);
            if (zifferOnly != null) {
        ...
        final int effectiveCount = (punkt1 != null) ? count - 1 : count;
        if (effectiveCount > 0)
     ...
     */
    private static XTextCursor getZifferOnly(final XTextRange par, final boolean includeNoTab) {
        final XParagraphCursor cursor = UNO.XParagraphCursor(par.getText().createTextCursorByRange(par.getStart()));
     ...
     */
    private static XTextRange getVerfuegungspunkt1(final XTextDocument doc) {
        ...
        if (frame != null) {
            final XTextCursor cursor = frame.getText().createTextCursorByRange(frame.getText());
     ...
     */
    private static String abdruckString(final int number) {
     ...
     */
    private static String romanNumber(final int i) {
        String number = "" + i + ".";
     ...
     */
    private static Vector<Verfuegungspunkt> scanVerfuegungspunkte(final XTextDocument doc) {
        final Vector<Verfuegungspunkt> verfuegungspunkte = new Vector<Verfuegungspunkt>();

        // Verfügungspunkt1 hinzufügen wenn verfügbar.
        final XTextRange punkt1 = getVerfuegungspunkt1(doc);
        if (punkt1 != null) {
            final Verfuegungspunkt original = new Verfuegungspunkt(L.m(romanNumbers[0] + " Original"));
        ...
        // genug isolieren um ein entsprechende Ticket bei OOo dazu aufmachen zu
        // können, da der Absturz nur sporadisch auftrat.
        final XParagraphCursor cursor = UNO.XParagraphCursor(doc.getText().createTextCursorByRange(
        ...
        if (cursor != null) do {
            ...
            if (isVerfuegungspunkt(cursor)) {
                final String heading = cursor.getString();
            ...
            if ((isZuleitungszeile(cursor) || isVerfuegungspunktMitZuleitung(cursor))
                ...
                final String zuleit = cursor.getString();
    ...
    public static List<VerfuegungspunktInfo> callPrintDialog(final XTextDocument doc) {
        ...
        final Vector<Verfuegungspunkt> vps = scanVerfuegungspunkte(doc);
        final Iterator<Verfuegungspunkt> iter = vps.iterator();
        while (iter.hasNext()) {
            final Verfuegungspunkt vp = iter.next();
            String text = L.m("Verfügungspunkt '%1'", vp.getHeading());
            final Iterator<String> zuleits = vp.getZuleitungszeilen().iterator();
            while (zuleits.hasNext()) {
                final String zuleit = zuleits.next();
                text += "\n  --> '" + zuleit + "'";
            }
            Logger.debug2(text);
        }
        ...
        final ConfigThingy conf = WollMuxFiles.getWollmuxConf();
        final ConfigThingy svdds = conf.query("Dialoge").query("SachleitendeVerfuegungenDruckdialog");
        ...
        try {
            final SyncActionListener s = new SyncActionListener();
            new SachleitendeVerfuegungenDruckdialog(printDialogConf, vps, s);
            final ActionEvent result = s.synchronize();
            final String cmd = result.getActionCommand();
            final SachleitendeVerfuegungenDruckdialog slvd = (SachleitendeVerfuegungenDruckdialog) result.getSource();
            if (SachleitendeVerfuegungenDruckdialog.CMD_SUBMIT.equals(cmd) && slvd != null) {
                if (slvd.getPrintOrderAsc()) {
                    ...
                    final List<VerfuegungspunktInfo> lList = new ArrayList<VerfuegungspunktInfo>();
                    final ListIterator<VerfuegungspunktInfo> lIt = slvd.getCurrentSettings().listIterator(slvd.getCurrentSettings().size());
     ...
     * @author Christoph Lutz (D-III-ITD-5.1)
     */
    public static int countVerfuegungspunkte(final XTextDocument doc) {
        if (doc != null)
     ...
     */
    public static void printVerfuegungspunkt(final XPrintModel pmod, final int verfPunkt,
            final boolean isDraft, final boolean isOriginal, final short copyCount) {
        final XTextDocument doc = pmod.getTextDocument();
        ...
        final XTextViewCursorSupplier suppl = UNO.XTextViewCursorSupplier(UNO.XModel(pmod.getTextDocument()).getCurrentController());
        ...
        final List<XTextSection> hidingSections = getSectionsFromPosition(pmod.getTextDocument(), setInvisibleRange);
        final HashMap<XTextSection, Boolean> sectionOldState = new HashMap<XTextSection, Boolean>();
        for (final XTextSection section : hidingSections)
            try {
                final boolean oldState = AnyConverter.toBoolean(UNO.getProperty(section, "IsVisible"));
        ...
        for (final XTextSection section : hidingSections) {
            final Boolean oldState = sectionOldState.get(section);
     ...
     */
    public static void workaround103137(final TextDocumentController documentController) {
        if (documentController == null || documentController.getModel().doc == null) return;
        final XTextDocument doc = documentController.getModel().doc;
        final XParagraphCursor cursor = UNO.XParagraphCursor(doc.getText().createTextCursorByRange(
                doc.getText().getStart()));
        ...
        if (punkt1 != null) {
            final XTextRange punkt1ZifferOnly = getZifferOnly(punkt1, true);
            if (punkt1ZifferOnly != null) UNO.hideTextRange(punkt1ZifferOnly, false);
        }
     ...
     * @author Christoph Lutz (D-III-ITD-5.1)
     */
    private static List<XTextSection> getSectionsFromPosition(final XTextDocument doc,
            final XTextRange pos) {
        final Vector<XTextSection> v = new Vector<XTextSection>();
        if (pos == null) return v;
        final XTextRangeCompare comp = UNO.XTextRangeCompare(pos.getText());
        if (comp == null) return v;
        final XTextSectionsSupplier suppl = UNO.XTextSectionsSupplier(doc);
        if (suppl == null) return v;

        final XNameAccess sections = suppl.getTextSections();
        final String[] names = sections.getElementNames();
        for (int i = 0; i < names.length; i++) {
            ...
            if (section != null) {
                try {
                    final int diff = comp.compareRegionStarts(pos, section.getAnchor());
     ...
     */
    private static XStyle getParagraphStyle(final XTextDocument doc, final String name) {
        XStyle style = null;

        final XNameContainer pss = getStyleContainer(doc, PARAGRAPH_STYLES);
        if (pss != null) try {
     ...
     */
    private static XStyle createParagraphStyle(final XTextDocument doc, final String name,
     ...
     *         das Absatzformat nicht definiert ist.
     */
    private static XStyle getCharacterStyle(final XTextDocument doc, final String name) {
        XStyle style = null;

        final XNameContainer pss = getStyleContainer(doc, CHARACTER_STYLES);
        if (pss != null) try {
            style = UNO.XStyle(pss.getByName(name));
        } catch (final java.lang.Exception e) {
        }
     ...
     *         erzeugt werden konnte.
     */
    private static XStyle createCharacterStyle(final XTextDocument doc, final String name,
            final String parentStyleName) {
        final XNameContainer pss = getStyleContainer(doc, CHARACTER_STYLES);
        XStyle style = null;
        try {
     ...
     */
    private static XNameContainer getStyleContainer(final XTextDocument doc,
     ...
     * @param doc
     */
    private static void createUsedStyles(final XTextDocument doc) {
        XStyle style = null;
        ...
        if (style == null) {
            style = createParagraphStyle(doc, ParaStyleNameZuleitungszeile, ParaStyleNameDefault);
            UNO.setProperty(style, "FollowStyle", ParaStyleNameDefault);
        ...
        if (style == null) {
            style = createCharacterStyle(doc, CharStyleNameRoemischeZiffer, CharStyleNameDefault);
            UNO.setProperty(style, "FollowStyle", CharStyleNameDefault);
            UNO.setProperty(style, "CharWeight", Float.valueOf(FontWeight.BOLD));
        }
    }
    ...
    private static String getCopyName() {
        ...
        final ConfigThingy conf = WollMuxFiles.getWollmuxConf();
        final ConfigThingy nan = conf.query("SachleitendeVerfuegungen").query("ABDRUCK_NAME");
        try {
            name = L.m(nan.getLastChild().toString());
            Logger.debug(L.m("Verwende ABDRUCK_NAME '%1'", name));
        } catch (final NodeNotFoundException x) {
        }
        return name;
    }
    ...
    private static String[] getNumbers() {
        ...
        final ConfigThingy conf = WollMuxFiles.getWollmuxConf();
        final ConfigThingy nan = conf.query("SachleitendeVerfuegungen").query("NUMBERS");
        try {
     ...
     */
    public static void deMuxSLVStyles(final XTextDocument doc) {
        if (doc == null) return;

        final HashMap<String, String> mapOldNameToNewName = new HashMap<String, String>();
        final XParagraphCursor cursor = UNO.XParagraphCursor(doc.getText().createTextCursorByRange(
                doc.getText().getStart()));
        if (cursor != null)
            do {
            ...
            String oldName = "";
            try {
            oldName = AnyConverter.toString(UNO.getProperty(cursor, "ParaStyleName"));
            } catch (final IllegalArgumentException e) {
            }
            ...
            final Object hidden = UNO.getProperty(cursor, "CharHidden");
        ...
        try {
            final XNamed frame = UNO.XNamed(UNO.XTextFramesSupplier(doc).getTextFrames().getByName(
                    FrameNameVerfuegungspunkt1));
