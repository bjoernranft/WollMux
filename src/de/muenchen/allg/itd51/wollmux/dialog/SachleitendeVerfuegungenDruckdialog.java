...
public class SachleitendeVerfuegungenDruckdialog {
     ...
     */
    private final ActionListener actionListener_printElement = new ActionListener() {
        @Override
     ...
     */
    private final ActionListener actionListener_printAll = new ActionListener() {
        @Override
        public void actionPerformed(final ActionEvent e) {
     ...
     */
    private final ActionListener actionListener_abort = new ActionListener() {
        @Override
        public void actionPerformed(final ActionEvent e) {
            abort(CMD_CANCEL);
        }
     ...
     */
    private final ItemListener cboxItemListener = new ItemListener() {
        @Override
        public void itemStateChanged(final ItemEvent arg0) {
            final Object source = arg0.getSource();
            if (source != null && source instanceof JComboBox<?>) {
                @SuppressWarnings("unchecked")
                final JComboBox<String> cbox = (JComboBox<String>) source;
     ...
     */
    private final JCheckBox printOrder;
     ...
     */
    private final ActionListener dialogEndListener;

    /**
     * Vector of Verfuegungspunkt, der die Beschreibungen der Verfügungspunkte enthält.
     */
    private final Vector<Verfuegungspunkt> verfuegungspunkte;
     ...
     */
    private final List<VerfuegungspunktInfo> currentSettings;
     ...
     *             Schlüssel fehlt.
     */
    public SachleitendeVerfuegungenDruckdialog(final ConfigThingy conf,
            final Vector<Verfuegungspunkt> verfuegungspunkte, final ActionListener dialogEndListener)
            throws ConfigurationErrorException {
        ...
        final ConfigThingy fensterDesc1 = conf.query("Fenster");
    ...
    public static class VerfuegungspunktInfo {
        ...
        public VerfuegungspunktInfo(final int verfPunktNr, final short copyCount, final boolean isDraft,
     ...
     * @author Matthias Benkmann (D-III-ITD 5.1), Christoph Lutz (D-III-ITD 5.1)
     */
    private void createGUI(final ConfigThingy fensterDesc) {
        Common.setLookAndFeelOnce();

        final int size = verfuegungspunkte.size();
        ...
        for (int i = 0; i < size; ++i) {
            final Verfuegungspunkt verfPunkt = verfuegungspunkte.get(i);
            final Vector<String> zuleitungszeilen = verfPunkt.getZuleitungszeilen();
            ...
            final Iterator<String> iter = zuleitungszeilen.iterator();
            while (iter.hasNext()) {
                final String zuleitung = iter.next();
                content.add(cutContent(zuleitung));
            }
            elementComboBoxes.add(new JComboBox<String>(content));

            // elementCountComboBoxes vorbelegen:
            final SpinnerNumberModel model = new SpinnerNumberModel(verfPunkt.getNumberOfCopies(), 0, 50, 1);
        ...
        final JPanel verfPunktPanel = new JPanel(new GridBagLayout());
        ...
        final GridBagConstraints gbcSeparator = new GridBagConstraints(0, 0, GridBagConstraints.REMAINDER, 1, 1.0, 0.0,
                GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL, new Insets(0, 0,
                        0, 0),
                0, 0);
        final JPanel uiElement = new JPanel(new GridLayout(1, 1));
        ...
        myFrame.pack();
        final int frameWidth = myFrame.getWidth();
        final int frameHeight = myFrame.getHeight();
        final Dimension screenSize = Toolkit.getDefaultToolkit().getScreenSize();
        final int x = screenSize.width / 2 - frameWidth / 2;
        final int y = screenSize.height / 2 - frameHeight / 2;
     ...
     * @author Christoph Lutz (D-III-ITD-5.1)
     */
    private static String cutContent(final String value) {
     ...
     */
    private void addUIElements(final ConfigThingy conf, final String key, final int verfPunktNr,
            final int yOffset, final JComponent compo, final int stepx, final int stepy) {
        ...
        final GridBagConstraints gbcLabel = new GridBagConstraints(0, 0, 1, 1, 0.0, 0.0, GridBagConstraints.LINE_START,
                GridBagConstraints.NONE, new Insets(TF_BORDER, TF_BORDER, TF_BORDER,
                        TF_BORDER),
                0, 0);
        final GridBagConstraints gbcGlue = new GridBagConstraints(0, 0, 1, 1, 1.0, 1.0, GridBagConstraints.LINE_START,
                GridBagConstraints.BOTH, new Insets(0, 0, 0, 0), 0, 0);
        final GridBagConstraints gbcButton = new GridBagConstraints(0, 0, 1, 1, 0.0, 0.0, GridBagConstraints.LINE_START,
                GridBagConstraints.NONE, new Insets(BUTTON_BORDER, BUTTON_BORDER,
                        BUTTON_BORDER, BUTTON_BORDER),
                0, 0);
        final GridBagConstraints gbcComboBox = new GridBagConstraints(0, 0, 1, 1, 1.0, 1.0, GridBagConstraints.CENTER,
                GridBagConstraints.BOTH, new Insets(TF_BORDER, TF_BORDER, TF_BORDER,
                        TF_BORDER),
                0, 0);
        final GridBagConstraints gbcTextField = new GridBagConstraints(0, 0, 1, 1, 1.0, 1.0, GridBagConstraints.CENTER,
                GridBagConstraints.BOTH, new Insets(TF_BORDER, TF_BORDER, TF_BORDER,
                        TF_BORDER),
                0, 0);
        final GridBagConstraints gbcSpinner = new GridBagConstraints(0, 0, 1, 1, 1.0, 1.0, GridBagConstraints.CENTER,
                GridBagConstraints.BOTH, new Insets(TF_BORDER, TF_BORDER, TF_BORDER,
                        TF_BORDER),
                0, 0);
        final GridBagConstraints gbcCheckBox = new GridBagConstraints(0, 0, 1, 1, 1.0, 1.0, GridBagConstraints.CENTER,
                GridBagConstraints.BOTH, new Insets(TF_BORDER, TF_BORDER, TF_BORDER,
                        TF_BORDER),
                0, 0);

        final ConfigThingy felderParent = conf.query(key);
        int y = -stepy + yOffset;
        int x = -stepx;

        final Iterator<ConfigThingy> piter = felderParent.iterator();
        while (piter.hasNext()) {
            final Iterator<ConfigThingy> iter = piter.next().iterator();
            while (iter.hasNext()) {
                y += stepy;
                x += stepx;

                final ConfigThingy uiElementDesc = iter.next();
                try {
                    /*
                    ...
                    final String type = uiElementDesc.get("TYPE").toString();

                    if (type.equals("label")) {
                        final JLabel uiElement = new JLabel();
                        gbcLabel.gridx = x;
                        gbcLabel.gridy = y;
                        final String labelText = uiElementDesc.get("LABEL").toString();
                        ...
                        final Box uiElement = Box.createHorizontalBox();
                        try {
                            final int minsize = Integer.parseInt(uiElementDesc.get("MINSIZE").toString());
                        ...
                        final String boxText = uiElementDesc.get("LABEL").toString();
                        ...
                        final String label = uiElementDesc.get("LABEL").toString();
                        ...
                        final ActionListener actionL = getAction(action);
     ...
     * @author Matthias Benkmann (D-III-ITD 5.1), Christoph Lutz (D-III-ITD 5.1)
     */
    private ActionListener getAction(final String action) {
     ...
     * @author Matthias Benkmann (D-III-ITD 5.1)
     */
    private void abort(final String cmdStr) {
        myFrame.dispose();
     ...
     * @author christoph.lutz
     */
    private void getCurrentSettingsForElement(final JButton button) {
        currentSettings.clear();
     ...
     * @author christoph.lutz
     */
    private VerfuegungspunktInfo getVerfuegungspunktInfo(final int verfPunkt) {
        // Anzahl der Kopien bestimmen:
        short numberOfCopies = 0;
        try {
            numberOfCopies = new Short(elementCountSpinner[verfPunkt - 1].getValue().toString()).shortValue();
        } catch (final Exception e) {
            Logger.error(L.m("Kann Anzahl der Ausfertigungen nicht bestimmen."), e);
        }

        final boolean isDraft = (verfPunkt == verfuegungspunkte.size());
        final boolean isOriginal = (verfPunkt == 1);
    ...
    public static void main(final String[] args) throws Exception {
        ...
        final List<VerfuegungspunktInfo> info = SachleitendeVerfuegung.callPrintDialog(UNO.XTextDocument(UNO.desktop.getCurrentComponent()));
        for (final VerfuegungspunktInfo v : info) {
