...
public class FormSidebarPanel extends AbstractSidebarPanel implements XToolPanel, XSidebarPanel {
	 ...
	 * 
	 * @param context               the sidebar context.
	 * @param parentWindow          parent sidebar window.
	 * @param resourceUrl           resource description.
	 * @param formSidebarController {@link FormSidebarController} the form sidebar
	 *                              controller.
	 */
	 ...
	 *
	 * @param config The UI configuration of the form.
	 * @param model  {@link FormModel} FormModel from @{link
	 *               {@link TextDocumentController}.
	 */
	 ...
	 * 
	 * @param tab                     {@link TabConfig} Tab from {@link FormModel}.
	 * @param tabPageControlContainer {@link XControlContainer} ControlContainer in
	 *                                which the controls will be inserted.
	 * @param layout                  The {@link Layout} in which newly created
	 *                                Layouts by this method will be inserted.
	...
	/**
	 * Add a control that performs the supplied action if it gets the focus. The
	 * action gets a {@link Stream} of the controls which are visible on the given
	 * tab.
	 *
	 * @param id                      The ID of the control.
	 * @param tab                     The tab which has the controls for the action.
	 * @param action                  The action.
	 * @param tabPageControlContainer {@link XControlContainer} ControlContainer in
	 *                                which the control will be inserted.
	 * @param layout                  The {@link Layout} in which in which the
	...
	private void addTabSwitcher(String id, TabConfig tab, Consumer<Stream<XWindow2>> action,
		...
		XControl tabSwithcer = GuiFactory.createTextfield(xMCF, context, id + tab.getId(), new Rectangle(0, 0, 0, 0),
				null, null);
			...
			public void focusGained(FocusEvent event) {
				if ((event.FocusFlags & FocusChangeReason.TAB) == 1) {
					Stream<XWindow2> controlConfigs = tab.getControls().stream()
							...
							.map(c -> controls.get(c.getId())).filter(Objects::nonNull)
							.map(p -> UNO.XWindow2(p.getRight())).filter(Objects::nonNull).filter(XWindow2::isVisible);
	 ...
	 * 
	 * @param tab                     Tab from {@link FormModel}
	 * @param tabPageControlContainer The ControlContainer where controls will be
	 *                                inserted.
	 * @param tabPageButtonVLayout    layout instance.
	...
	/**
	 * Creates LO's UI controls by type of {@link UIElementConfig}. And puts them in
	 * a Layout.
	 * 
	 * @param control control model.
	 * @param page    {@link XTabPage} as XControl
	...
	private Layout createXControlByType(UIElementConfig control, XControlContainer pageContainer) {
		...
		case BUTTON:
			...
			xControl = GuiFactory.createButton(xMCF, context, htmlElement.getText(),
					formSidebarController::buttonPressed, new Rectangle(0, 0, 150, 20), props);
		...
		case LABEL:
			...
			SortedMap<String, Object> propsGraphic = new TreeMap<>();
			propsGraphic.put(UnoProperty.DEFAULT_CONTROL, control.getId());
			propsGraphic.put("BackgroundColor", Color.BLUE.getRGB() & ~0xFF000000);
			propsGraphic.put("Border", (short) 0);
			//propsGraphic.put("ScaleImage", Boolean.TRUE);
			propsGraphic.put("ScaleMode", ImageScaleMode.ANISOTROPIC);
			xLabel = GuiFactory.createGraphicsControl(xMCF, context, new Rectangle(0, 0, 150, 60), propsGraphic);
			
			UNO.XWindow(xLabel).addWindowListener(new XWindowListener() {
				...
				public void windowResized(WindowEvent arg0) {
					// TODO Auto-generated method stub
					try {
						XWindow wnd = UNO.XWindow(arg0.Source);
						
						byte[] image = genTextImage(htmlElement, wnd.getPosSize());
						XGraphic graphic = genXGraphic(control, image);
						XControl xControl = UNO.XControl(arg0.Source);
						XPropertySet prop = UNO.XPropertySet(xControl.getModel());
						prop.setPropertyValue("Graphic", graphic);

					} catch (IOException | IllegalArgumentException | PropertyVetoException
							| WrappedTargetException | UnknownPropertyException e) {
		...
		case TEXTAREA:
			...
			props.put(UnoProperty.READ_ONLY, control.isReadonly());
			props.put(UnoProperty.HELP_TEXT, control.getTip());
			xControl = GuiFactory.createTextfield(xMCF, context, "", new Rectangle(0, 0, 100, control.getLines() * 20),
					props, formSidebarController::textChanged);
			UNO.XWindow(xControl).addFocusListener(formSidebarController.getFocusListener());
			break;
	...
	private XGraphic genXGraphic(UIElementConfig control, byte[] image) {
		com.sun.star.graphic.XGraphicProvider xGraphicProvider;
		XGraphic graphic = null;

		try {
			xGraphicProvider = UnoRuntime.queryInterface(com.sun.star.graphic.XGraphicProvider.class,
					xMCF.createInstanceWithContext("com.sun.star.graphic.GraphicProvider", this.context));
			PropertyValue[] propValues = new PropertyValue[] { new PropertyValue() };
			propValues[0].Name = "InputStream";
			propValues[0].Value = new ByteArrayToXInputStreamAdapter(image);

			try {
				graphic = xGraphicProvider.queryGraphic(propValues);
			} catch (IllegalArgumentException | com.sun.star.io.IOException | WrappedTargetException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
	...
	private byte[] genTextImage(HTMLElement htmlElement, Rectangle rect) throws IOException {
		//String txt = "hfshsdfhsdfhsdfhh \n DFDSFDSFDF";
		String txt = htmlElement.getText();
		
		List<AttributedString> attrStrings = new ArrayList<>();
		for (String line : txt.split("\n")) {
			AttributedString text = new AttributedString(line);
			int fontSize = htmlElement.getFontDescriptor().Height == 0 ? 12 : htmlElement.getFontDescriptor().Height;
			text.addAttribute(TextAttribute.FONT, new Font("Arial", Font.BOLD, fontSize), 0, line.length());
			text.addAttribute(TextAttribute.FOREGROUND, Color.BLACK, 0, line.length());
			text.addAttribute(TextAttribute.BACKGROUND, Color.WHITE, 0, line.length());
			attrStrings.add(text);
		}
		...
		try (ImageOutputStream output = new MemoryCacheImageOutputStream(bytes)) {
			...
			Canvas c = new Canvas();
			FontMetrics fmCanvas = c.getFontMetrics(font);
			System.out.println(fmCanvas.stringWidth(txt));
			System.out.println(rect.Width);
			if (fmCanvas.stringWidth(txt) > rect.Width)
			{
				// calc new height of image due linebreaks, set new image control height
			}
			...
			int y = 0;
			for (AttributedString line : attrStrings) {
				LineMetrics lm = fm.getLineMetrics(line.getIterator(), 0, line.getIterator().getEndIndex(), graphics);
				graphics.drawString(line.getIterator(), 0, y += (int)lm.getHeight());
			}
	...
	private XControl createLabel(UIElementConfig control, HTMLElement htmlElement) {
		if (htmlElement != null) {
			...
			if (!htmlElement.getHref().isEmpty()) {
				...
				return GuiFactory.createHyperLinkLabel(xMCF, context, htmlElement.getText(),
						new Rectangle(0, 0, 100, 20), props);
			} else {
				return GuiFactory.createLabel(xMCF, context, htmlElement.getText(), new Rectangle(0, 0, 100, 20),
						props);
			}
		} else {
	 ...
	 * 
	 * @param id   {@link XControlModel} Model from {@link XControl}
	...
	public void previousTab() {
		if (tabChanged) {
			...
			while (prev > 0 && !UNO.XTabPageModel(UNO.XControl(tabControlContainer.getTabPageByID(prev)).getModel())
					.getEnabled()) {
				prev--;
			}
	 ...
	 *
	 * @param id    The ID of the control.
	 * @param okay  If true the control gets its default background color, otherwise
	 *              the provided color.
	 * @param color The background color to set if okay is false.
	 */
